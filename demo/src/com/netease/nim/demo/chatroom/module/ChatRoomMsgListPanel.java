...
public class ChatRoomMsgListPanel {
    ...
    private MediaPlayer startAlarmMusic;
    private MediaPlayer stopAlarmMusic;
            ...
            if (isMyMessage(message)) {
                if (!TextUtils.isEmpty(message.getContent())) {
                    ...
                    switch (message.getContent()) {
                        case "000000X":
                            AVChatSoundPlayer.instance().play(AVChatSoundPlayer.RingerTypeEnum.STOP);
                            new AlertDialog.Builder(container.activity).setTitle("提醒")//设置对话框标题

                                    .setMessage("同学们下课了！")//设置显示的内容
                            ...
                            break;
                        case "000000Y":
                            AVChatSoundPlayer.instance().play(AVChatSoundPlayer.RingerTypeEnum.START);
                            new AlertDialog.Builder(container.activity).setTitle("提醒")//设置对话框标题

                                    .setMessage("大家注备好开始上课了！")//设置显示的内容
    ...
    private class MessageLoader implements BaseFetchLoadAdapter.RequestLoadMoreListener, BaseFetchLoadAdapter.RequestFetchMoreListener {
        ...
        private void onMessageLoaded(List<ChatRoomMessage> messages) {
            ...
            if (count <= 0) {
                ...
                List<ChatRoomMessage> tempmessages = new ArrayList<ChatRoomMessage>();
                for (ChatRoomMessage message : messages) {
                    if (!TextUtils.isEmpty(message.getContent())) {
                        switch (message.getContent()) {
                            case "000000X":
                ...
                messages.removeAll(tempmessages);
