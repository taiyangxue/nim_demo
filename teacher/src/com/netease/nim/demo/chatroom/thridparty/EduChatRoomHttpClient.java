...
//                        callback.onSuccess(roomAddrs);
     ...
     * @param callback
     */
    public void getVideoList(int typeid,int pageSize,final ChatRoomHttpClient.ChatRoomHttpCallback<VideoListResult.RetBean> callback) {
...
//        jsonObject.put("type", 0);
            ...
            public void onResponse(String response, int code, Throwable e) {
                ...
                try {
                    if (result.getCode() == RESULT_CODE_SUCCESS) {
                        // reply
                        callback.onSuccess(result.getRet());
                    } else {
                        callback.onFailed(code, null);
                    }
                } catch (JSONException e1) {
     ...
     * @param callback
     */
    public void deleteVideoList(int vid,final ChatRoomHttpClient.ChatRoomHttpCallback<String> callback) {
        String url = DemoServers.API_SERVER2 + "vod/video/videoDelete";
        Map<String, String> headers = new HashMap<>(2);
        String appKey = readAppKey();
        String curTime = String.valueOf((new Date()).getTime() / 1000L);
        String checkSum = CheckSumBuilder.getCheckSum(APP_SECRET, NONCE, curTime);//参考 计算CheckSum的java代码
